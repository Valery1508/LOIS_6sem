    ...
    public Parser(StringBuilder formula) {
        ...
        if (isFormula(formula)) {
            ...
            for (String subFormula : subFormulaList) {
                if (subFormula.startsWith(LEFT_BRACKET)) {
                    subListOfSecondPart.add(subFormula);
                } else {
                    subListOfAtomicAndConstant.add(subFormula);
                }
            }

            countOfAtomicAndConstant = subListOfAtomicAndConstant.size();
            for (String subFormula : subListOfAtomicAndConstant) {
                if (subFormula.equals(LOGIC_FALSE) || subFormula.equals(LOGIC_TRUE)) {
                    countOfConstants++;
                }
            }
            ...
            SDNFConstructor sdnfConstructor = new SDNFConstructor();
            sdnfConstructor.generateSDNF(countOfAtomicAndConstant, tableMap);

        } else {
            System.out.println("Данная строка НЕ является формулой языка логики высказываний");
        }
